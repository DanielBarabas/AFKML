...
with exp:
    ...
    if len(options) == 2:
        st.write("Ok")
        #Both category:
        if small_df[options[0]].dtype in cat_list and small_df[options[1]].dtype in cat_list:
            fig = stacked_bar(small_df,options)
        #Both continous:
        elif small_df[options[0]].dtype not in cat_list and small_df[options[1]].dtype not in cat_list:
